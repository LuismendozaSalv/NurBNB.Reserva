...
using NURBNB.Alojamiento.Application.UseCases.Reserva;
...
namespace NURBNB.Alojamiento.WebAPI.Controllers
{
    ...
    public class PropiedadController : ControllerBase
    {
        ...
        public async Task<IActionResult> AgregarReserva([FromBody] AgregarReservaCommand command)
        {
            try
            {
                var propiedadId = await _mediator.Send(command);
        ...
        {
            var reservas = await _mediator.Send(new GetReservaListQuery()
            {
            });
            return Ok(reservas);
        }

        [HttpGet("reservasbypropId")]
        public async Task<IActionResult> getReservasByPropId(Guid searchTerm)
        {
            var reservas = await _mediator.Send(new GetReservaByPropiedadIdQuery()
            {
